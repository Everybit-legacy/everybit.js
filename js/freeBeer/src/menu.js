...
var IdentityMenu = React.createClass({
    ...
    render: function() {
        var currUser = PB.M.PuffWardrobe.getCurrentUsername();
...
var AuthorPicker = React.createClass({
    ...
    handleRemoveUser: function() {
        ...
        PB.M.PuffWardrobe.removeKeys(userToRemove);
        events.pub('user/'+userToRemove+'/remove', {});
        var all_usernames = Object.keys(PB.M.PuffWardrobe.getAll()).filter(function(u){return u!=userToRemove});
        if (all_usernames.length != 0) {
            PB.M.PuffWardrobe.switchCurrent(all_usernames[0]);
        }
        ...
        prom.then(function(userInfo) {
            ...
            if(publicKey != userInfo[keyType]) {
                ...
                if(keyType == 'defaultKey') {
                    PB.M.PuffWardrobe.storeDefaultKey(username, privateKey);
                }

                if(keyType == 'adminKey') {
                    PB.M.PuffWardrobe.storeAdminKey(username, privateKey);
                }

                if(keyType == 'rootKey') {
                    PB.M.PuffWardrobe.storeRootKey(username, privateKey);
                }
...
var ViewIdentity = React.createClass({
    ...
    handleClickQRCode: function(){
        // create the qr code
        var key = PB.M.PuffWardrobe.getCurrentKeys()[this.state.qrCode];
        var qr = qrcode(4, 'M');
        qr.addData(key);
        qr.make();
    ...
    render: function() {
        ...
        if (showQRCode) {
            ...
            var key = PB.M.PuffWardrobe.getCurrentKeys()[keyType];
            if (key.length < 1) {
                showQRCode = false;
            } else {
                var qr = qrcode(4, 'M');
    ...
    toggleShowRootKey: function() {
        if(this.state.rootKey == 'hidden') {
            this.setState({rootKey: PB.M.PuffWardrobe.getCurrentUserRecord().rootKey});
        } else {
...
var NewIdentity = React.createClass({
    ...
    handleUsernameRequest: function() {
        ...
        prom.then(function(userRecord) {
                ...
                PB.M.PuffWardrobe.storePrivateKeys(requestedUsername, rootKeyPrivate, adminKeyPrivate, defaultKeyPrivate);
                self.setState({step: 3,
                    errorMessage: polyglot.t("menu.identity.newIdentity.success")});

                // Set this person as the current user
                PB.M.PuffWardrobe.switchCurrent(requestedUsername);
                events.pub('ui/event', {});
            },
            function(err) {
                console.log("ERR")
